...
cat <<EOF > ./.tune_password_pattern_enc
    sambaoptsx="\$(echo "\$sambaoptsx" | openssl enc -e -aes-256-cbc -salt -iter $iter -base64 -A -k '$password' -in - -out -)" # SKUF_OPENSSL_ENC_RM #
EOF

cat <<EOF > ./.tune_password_pattern_dec
        if _sambaopts="\$(echo "\$_sambaopts" | openssl enc -d -aes-256-cbc -salt -iter $iter -base64 -A -k '$password' -in - -out -)"; then # SKUF_OPENSSL_DEC_RM #
EOF
